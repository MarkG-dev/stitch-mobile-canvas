        ...
        <div className="relative">
          ...
          loop: true
            ...
            <CarouselContent>
              {features.map((feature, index) => <CarouselItem key={index} className="md:basis-full">
                  <div className={cn("relative neon-border overflow-hidden", current === index ? "bg-neon/5" : "")} style={{
                width: '350px',
                height: '390px'
              }}>
          ...
          <div className="absolute left-6 top-1/2 -translate-y-1/2 z-10 flex flex-col gap-2">
            {features.map((_, index) => <button key={index} onClick={() => scrollTo(index)} className="focus:outline-none" aria-label={`Go to slide ${index + 1}`}>
                {current === index ? <CircleDot className="w-5 h-5 text-neon" /> : <Circle className="w-5 h-5 text-neon/50 hover:text-neon/70 transition-colors" />}
              </button>)}
          </div>
        </div>
      </Container>
